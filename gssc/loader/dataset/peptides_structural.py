...
class PeptidesStructuralDataset(InMemoryDataset):
    def __init__(self, root='datasets', smiles2graph=smiles2graph,
                 transform=None, pre_transform=None, replace=False, ymean_log=0.0, ystd_log=1.0):
        """
        ...
        self.ymin = data.y.min(dim=0, keepdim=True)[0]
        data.log_y = torch.log10(data.y - self.ymin + 1.0)

        if replace:
            data.y = (data.log_y - ymean_log) / ystd_log
        self.data, self.slices = data, slices
        self.log_mean, self.log_std = ymean_log, ystd_log
