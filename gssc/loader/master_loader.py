...
from gssc.loader.planetoid import Planetoid
from gssc.loader.dataset.aqsol_molecules import AQSOL
from gssc.loader.dataset.coco_superpixels import COCOSuperpixels
from gssc.loader.dataset.malnet_tiny import MalNetTiny
from gssc.loader.dataset.voc_superpixels import VOCSuperpixels
from gssc.loader.dataset.count_cycle import CountCycle
from gssc.loader.split_generator import (prepare_splits,
                                             set_dataset_splits)
from gssc.transform.posenc_stats import compute_posenc_stats
from gssc.transform.transforms import (pre_transform_in_memory,
                                           generate_splits, # not the same as split_generator above.
                                           typecast_x, concat_x_and_pos,
                                           clip_graphs_to_size, move_node_feat_to_x)
from gssc.transform.expander_edges import generate_random_expander
from gssc.transform.dist_transforms import (add_dist_features, add_reverse_edges,
...
def load_dataset_master(format, name, dataset_dir):
    ...
    elif format == 'count_cycle':
        dataset = preformat_count_cycle(dataset_dir, name)
    ...
    if pe_enabled_list:
        ...
        pre_transform_in_memory(dataset,
                                partial(compute_posenc_stats,
                                        ...
                                        cfg=cfg),
                                show_progress=True,
                                check_if_pe_done=cfg.extra.check_if_pe_done,
                                )
...
def preformat_count_cycle(dataset_dir, name):
    sname_mapping = {"3": 0, "4": 1, "5": 2, "6": 3}
    sname = name[len("count_cycle"):]
    yidx = sname_mapping[sname]
    trn_ds = CountCycle("count_cycle", split="train", yidx=yidx, root=dataset_dir)
    val_ds = CountCycle("count_cycle", split="val", yidx=yidx, root=dataset_dir)

    y_train_val = torch.cat([trn_ds.data.y, val_ds.data.y], dim=0)
    mean = y_train_val.mean()
    std = y_train_val.std()

    y_train_val_log = torch.cat([trn_ds.data.log_y, val_ds.data.log_y], dim=0)
    log_mean = y_train_val_log.mean()
    log_std = y_train_val_log.std()

    replace = True if cfg.extra.log_norm else False
    trn_ds = CountCycle("count_cycle", split="train", yidx=yidx, ymean=mean, ystd=std, root=dataset_dir, ymean_log=log_mean, ystd_log=log_std, replace=replace)
    val_ds = CountCycle("count_cycle", split="val", yidx=yidx, ymean=mean, ystd=std, root=dataset_dir, ymean_log=log_mean, ystd_log=log_std, replace=replace)
    tst_ds = CountCycle("count_cycle", split="test", yidx=yidx, ymean=mean, ystd=std, root=dataset_dir, ymean_log=log_mean, ystd_log=log_std, replace=replace)

    trn_ds.num_tasks = 1
    val_ds.num_tasks = 1
    tst_ds.num_tasks = 1

    dataset = join_dataset_splits([trn_ds, val_ds, tst_ds])
    return dataset
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == 'ogbg-ppa':
        ...
        from gssc.loader.ogbg_code2_utils import idx2vocab, \
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from gssc.loader.dataset.pcqm4mv2_contact import \
...
def preformat_Peptides(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary.
        from gssc.loader.dataset.peptides_functional import \
            PeptidesFunctionalDataset
        from gssc.loader.dataset.peptides_structural import \
