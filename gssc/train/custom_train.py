...
from gssc.loss.subtoken_prediction_loss import subtoken_cross_entropy
from gssc.utils import cfg_to_dict, flatten_dict, make_wandb_name
...
def eval_epoch(logger, loader, model, split='val'):
    ...
    for batch in loader:
        ...
        if cfg.dataset.name == 'ogbg-code2':
            ...
            if "count_cycle" in cfg.dataset.name:
                if cfg.extra.log_norm:
                    ystd_log = loader.dataset.log_std
                    ymean_log = loader.dataset.log_mean
                    ymean = loader.dataset.mean
                    ystd = loader.dataset.std
                    pred = ((10**((pred * ystd_log) + ymean_log) - 1) - ymean) / ystd
                    true = ((10**((true * ystd_log) + ymean_log) - 1) - ymean) / ystd
