...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    if 'LapPE' in pe_types or 'EquivStableLapPE' in pe_types:
        ...
        adj = SparseTensor.from_edge_index(undir_edge_index, sparse_sizes=(N, N))
        deg = adj.sum(dim=1)
        ...
        # num_eig = 64
        # rw_evals, rw_evects = torch.linalg.eigh(adj)
        # largest_eig_index = torch.argsort(rw_evals, descending=True)[:num_eig]
        # rw_evals = rw_evals[largest_eig_index]
        # rw_evects = rw_evects[:, largest_eig_index]
        # # res = rw_evects @ torch.diag_embed(rw_evals) @ rw_evects.T

        # if N < num_eig:
        #     rw_evects = F.pad(rw_evects, (0, num_eig - N), value=0.0)
        # else:
        #     rw_evects = rw_evects

        # # Pad and save eigenvalues.
        # if N < num_eig:
        #     rw_evals = F.pad(rw_evals, (0, num_eig - N), value=0.0).unsqueeze(0)
        # else:
        #     rw_evals = rw_evals.unsqueeze(0)
        # rw_evals = rw_evals.repeat(N, 1).unsqueeze(2)
        # data.rw_evals = rw_evals
        # data.rw_evects = rw_evects
