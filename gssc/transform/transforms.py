...
def pre_transform_in_memory(dataset, transform_func, show_progress=False, check_if_pe_done=False):
    ...
    if check_if_pe_done:
        file_name = dataset.processed_dir + "/pe_transformed.pt"
        if os.path.exists(file_name):
            print(f"Loading pre-transformed PE from {file_name}")
            data_list, collate_data, collate_slices = torch.load(file_name)
            dataset._indices, dataset._data_list, dataset.data, dataset.slices = None, data_list, collate_data, collate_slices
            print(f"Loaded pre-transformed PE from {file_name}")
        else:
            print(f"Pre-transforming PE and saving to {file_name}")
            ...
            torch.save((data_list, dataset.data, dataset.slices), file_name)
            print(f"Saved pre-transformed PE to {file_name}")
    else:
        data_list = [transform_func(dataset.get(i))
                    for i in tqdm(range(len(dataset)),
                                disable=not show_progress,
